...
def adddevice():
    logger.info("Received request for geofence endpoint")
    ...
    try: 
        ...
        with get_db_conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    "INSERT INTO addevice(device_name,device_id,serial_number) VALUES (%s, %s, %s)",
                    (device_name, device_id,serial_number)
                )
                conn.commit()
                logger.info(f"Successfully inserted adddevice data for device_id={device_id}, device_name={device_name}, serial_number={serial_number}")

        return jsonify({"status": "success"})
    except Exception as e:
        logger.error(f"Error processing adddevice data: {str(e)}")
